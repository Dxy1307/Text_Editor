...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
	...
	public boolean addWord(String word)
	{
		...
		char[] charInWord = lowerWord.toCharArray();
		
		for(int i = 0; i < charInWord.length; i++) {
			char c = charInWord[i];
			if(prevNode.getChild(c) == null) {
				prevNode = prevNode.insert(c);
			} else {
				prevNode = prevNode.getChild(c);
			}
			
			if(i == (charInWord.length - 1) && !prevNode.endsWord()) {
	...
	public boolean isWord(String s) 
	{
		...
		char[] charInS = lowerS.toCharArray();
		
		TrieNode prevNode = root;
		for(int i = 0; i < charInS.length; i++) {
			prevNode = prevNode.getChild(charInS[i]);
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 for(char c : chars) {
    		 prevNode = prevNode.getChild(c);
    		 if(prevNode == null) return completions;
    	 }
